<?php
	...
	@ Notices
	******************************/
	//shton njoftim të ri nga paneli
	...
	//shfaq të dhënat e njoftimit në panelin e kontrollit
	...
	//njoftimi i azhurnimit nga paneli
	...
	******************************/
	...
	//shfaq të dhënat e eventit në panelin e kontrollit
	...
	******************************/
		...
		if(isset($_POST['user_login'])) {
			...
			if(!empty($username) && !empty($user_password) && !empty($user_role)) {
				...
				if($username == $db_username && $user_password == $db_user_password && $user_role == $db_user_role) {
					...
					$error = "The data is not correct, please try again.";
				}
			} else {
	...
	// nëse përdoruesi është i loguar në sistem
	...
	// nëse admini është i loguar në sistem
	function is_admin() {
	...
	// nëse nxënësi është i loguar në sistem
	function is_student() {
	...
	// nëse mësuesi është i loguar në sistem
	function is_teacher() {
	...
	@ Teacher functions
	******************************/
	//regjistron mësuesin nga admini (drejtori)
	...
	//këtu shfaqën mësuesit në panelin e ndryshimit
	...
	//merr emailin e mësuesit me ID-në e mësuesit
	...
	******************************/
	...
	// ndrysho/azhuro nxënësit përmes ID-së
	...
	//merrni emailin e nxënësit me ID-në e nxënsit
	...
	***************************************/
	...
	function add_section() {
		...
		if(isset($_POST['section_submit'])) {
			...
			$group_name = clean_data($_POST['group_name']);

			$grp_available = empty($group_name) ? '' : " | Group: ".$group_name;

			$global_name = "Class: {$class_id} | Section: $section $grp_available";
			if(!empty($class_id) && !empty($section)) {
				$query = "INSERT INTO sections (class_id, section, group_name, global_name) VALUES ('$class_id', '$section', '$group_name', '$global_name')";
				$result = mysqli_query($conn, $query);
				if($result) {
	...
	function delete_section() {
		...
		if(isset($_GET['delete_section'])) {
			...
			$student_group = get_group_by_global_id($id);

			$delete_class_query = "DELETE FROM students WHERE student_class=$class AND student_section='$student_section' AND student_group='$student_group'";
	...
	function get_group_by_global_id($global_id) {
		global $conn;
		$query = "SELECT group_name FROM sections WHERE id=$global_id";
		$result = mysqli_query($conn, $query);
		$data = mysqli_fetch_assoc($result);
	...
	function get_school_address() {
		...
		if(mysqli_num_rows($result) > 0) {
			...
			$school_addr = 'Nerashte, North Macedonia';
		}
		return $school_addr;
	}
	...
	****************************************/
	...
	///tranferon nxënësit në klasë tjeter
	...
	//funksioni për studentët e dështuar
	...
	//kontrolloni nëse tabela e rezultateve të vlerësimit është bosh
